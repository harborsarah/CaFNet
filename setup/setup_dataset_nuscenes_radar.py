...
TRAIN_RADAR_NEW_FILEPATH = os.path.join(
    TRAIN_REF_DIRPATH, 'nuscenes_train_radar_new.txt')
TRAIN_RADAR_REPROJECTED_NEW_FILEPATH = os.path.join(
    TRAIN_REF_DIRPATH, 'nuscenes_train_radar_reprojected_new.txt')
TRAIN_BOX_POS_FILEPATH = os.path.join(
    TRAIN_REF_DIRPATH, 'nuscenes_train_box_pos.txt')

VAL_RADAR_NEW_FILEPATH = os.path.join(
    VAL_REF_DIRPATH, 'nuscenes_val_radar_new.txt')
VAL_RADAR_REPROJECTED_NEW_FILEPATH = os.path.join(
    VAL_REF_DIRPATH, 'nuscenes_val_radar_reprojected_new.txt')
VAL_BOX_POS_FILEPATH = os.path.join(
    VAL_REF_DIRPATH, 'nuscenes_val_box_pos.txt')

VAL_RADAR_NEW_SUBSET_FILEPATH = os.path.join(
    VAL_REF_DIRPATH, 'nuscenes_val_radar_new-subset.txt')
VAL_RADAR_REPROJECTED_NEW_SUBSET_FILEPATH = os.path.join(
    VAL_REF_DIRPATH, 'nuscenes_val_radar_reprojected_new-subset.txt')
VAL_BOX_POS_SUBSET_FILEPATH = os.path.join(
    VAL_REF_DIRPATH, 'nuscenes_val_box_pos-subset.txt')
...
if __name__ == '__main__':
    ...
    train_radar_points_new_paths = []
    train_radar_points_reprojected_new_paths = []
    train_box_pos_paths = []

    val_radar_points_new_paths = []
    val_radar_points_reprojected_new_paths = []
    val_box_pos_paths = []
    ...
    for results in pool_results:
        ...
        if tag == 'train':
            train_radar_points_new_paths.extend(radar_points_scene_paths)
            train_radar_points_reprojected_new_paths.extend(radar_points_reprojected_scene_paths)
            train_box_pos_paths.extend(box_pos_scene_paths)
        elif tag == 'val':
            val_radar_points_new_paths.extend(radar_points_scene_paths)
            val_radar_points_reprojected_new_paths.extend(radar_points_reprojected_scene_paths)
            val_box_pos_paths.extend(box_pos_scene_paths)
        else:
            raise ValueError('Found invalid tag: {}'.format(tag))

    # Subsample from validation set
    val_radar_points_new_subset_paths = val_radar_points_new_paths[::2]
    val_radar_points_reprojected_new_subset_paths = val_radar_points_reprojected_new_paths[::2]
    val_box_pos_subset_paths = val_box_pos_paths[::2]
        ...
        [
            ...
            [
                [
                    'radar new',
                    train_radar_points_new_paths,
                    TRAIN_RADAR_NEW_FILEPATH
                ], [
                    'radar reprojected new',
                    train_radar_points_reprojected_new_paths,
                    TRAIN_RADAR_REPROJECTED_NEW_FILEPATH,
                ], [
                    'box mask',
                    train_box_pos_paths,
                    TRAIN_BOX_POS_FILEPATH,
                ],
            ]
        ], [
            'validation',
            [
                [
                    'radar new',
                    val_radar_points_new_paths,
                    VAL_RADAR_NEW_FILEPATH
                ], [
                    'radar_reprojected',
                    val_radar_points_reprojected_new_paths,
                    VAL_RADAR_REPROJECTED_NEW_FILEPATH
                ], [
                    'box mask',
                    val_box_pos_paths,
                    VAL_BOX_POS_FILEPATH
                ], [
                    'radar new subset',
                    val_radar_points_new_subset_paths,
                    VAL_RADAR_NEW_SUBSET_FILEPATH
                ], [
                    'radar reprojected new subset',
                    val_radar_points_reprojected_new_paths,
                    VAL_RADAR_REPROJECTED_NEW_SUBSET_FILEPATH
                ], [
                    'box mask subset',
                    val_box_pos_subset_paths,
                    VAL_BOX_POS_SUBSET_FILEPATH
                ]
            ]
        ]
    ]
